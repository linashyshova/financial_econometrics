...
ggplot(hsi_data, aes(x = date, y = price)) +
  ...
  theme_minimal()
#ggsave("hsi_index_price.png", width = 8, height = 5, dpi = 300)
...
ggplot(hsi_data, aes(x = date, y = log_return)) +
  ...
  theme_minimal()
#ggsave("hsi_index_log_return.png", width = 8, height = 5, dpi = 300)

# ACF for weekly log returns
autoplot(acf(na.omit(hsi_data$log_return), plot = FALSE)) +
  ggtitle("ACF of HSI log-returns") + 
  theme_minimal()
#ggsave("acf_plot.png", width = 8, height = 5, dpi = 300)

# ACF for weekly squared log returns
autoplot(acf((na.omit(hsi_data$log_return))^2, plot = FALSE)) +
  ggtitle("ACF of HSI squared log-returns") + 
  theme_minimal()
#ggsave("squared_acf_plot.png", width = 8, height = 5, dpi = 300)
...
ggplot(hsi_data, aes(x = date, y = filter_var)) +
  ...
  theme_minimal()
#ggsave("filter_var_garch12.png", width = 8, height = 5, dpi = 300)


############################## Question 4 ###############################
# Homoskedasticity test by plotting the ACF of squared residuals
u <- y/sqrt(hsi_data$filter_var)
autoplot(acf(u^2, plot = FALSE)) +
  ggtitle("ACF of squared residuals") + 
  theme_minimal()
#ggsave("resid2_acf_plot.png", width = 8, height = 5, dpi = 300)
...
ggplot(hsi_data, aes(x = date, y = filter_var_best)) +
  ...
  theme_minimal()
#ggsave("filter_var_bestmodel.png", width = 8, height = 5, dpi = 300)
